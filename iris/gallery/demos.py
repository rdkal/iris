...
*and* a doc panel on the tests gallery page — live iframe, routes, and source.
"""
...
def _():
    return browser_test(
        view=h.div[
            h.div("#panel")[h.p(".muted")["Nothing loaded yet."]],
            Button(fx_action="/content", fx_target="#panel", fx_swap="innerHTML")["Load content"],
        ],
        routes={"/content": Card[h.h3["Loaded!"], h.p["This arrived from the (stub) server."]]},
        steps=[
...
def _():
    return browser_test(
        view=Button(fx_action="/like", fx_swap="outerHTML")["♡ Like"],
        routes={"/like": Button(".primary", disabled=True)["♥ Liked"]},
        steps=[
            expect_text("♡ Like"),
            click("button"),
            expect_text("♥ Liked"),
...
def _():
    return browser_test(
        view=h.div[
            List(id="items", items=["Item 1", "Item 2"]),
            Button(fx_action="/more", fx_target="#items", fx_swap="beforeend")["Load more"],
        ],
        routes={"/more": h.fragment[h.li["Item 3"], h.li["Item 4"]]},
        steps=[
            expect_text("Item 2"),
...
def _():
    return browser_test(
        view=h.form(fx_action="/submit", fx_method="post", fx_target="#result", fx_swap="innerHTML")[
            h.input(name="email", placeholder="you@example.com"),
            Button(".primary", type="submit")["Sign up"],
            h.div("#result"),
        ],
        routes={"/submit": Banner(".success")["Thanks for signing up!"]},
        steps=[
